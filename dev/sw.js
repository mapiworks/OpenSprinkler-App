...
// Keep caches isolated for each deployed UI path (for example, / and /dev/).
// The same GitHub Pages origin can host both at once.
var cachePrefix = "OpenSprinkler-" + encodeURIComponent( self.registration.scope ) + "-";
...
// This worker is installed below the UI root (e.g. /OpenSprinkler-App/dev/).
// Resolve the manifest relative to that scope rather than the domain root.
cacheFiles = cacheFiles.map( function( file ) {
	return file.replace( /^\//, "" );
} );
